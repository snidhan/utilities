...
# GRID (include all this in the subroutine)
...
#xedges = xe
xedges = xc[1:-1] #this removes the first and the last element since removing just the first 
yedges = rc[1:-1] #would be [1:]
...
data_ng = data[1:-1,1:-1,1:-1]
data_slice = data_ng[:,0,:]

print('data=',data.shape)
print('data_ng=',data_ng.shape)
print('data_slice=',data_slice.shape)
plt.pcolormesh(X,Y, np.transpose(data_slice))
plt.colorbar
plt.show()
#fig=plt.figure();ax=fig.add_subplot(111, projection='polar');ax.pcolormesh(X, Y, data_slice)
#plt.show()
